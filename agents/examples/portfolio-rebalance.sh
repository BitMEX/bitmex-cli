...
# Requirements: bitmex (on PATH), jq, bc
set -euo pipefail
...
bitmex paper init --balance "$INITIAL_BALANCE" -o json 2>/dev/null | jq '.'
echo ""

# Get prices
echo "Step 2: Fetch prices"
BTC_PRICE=$(bitmex ticker BTCUSD -o json 2>/dev/null | jq -r '.[].c[0] // "0"')
ETH_PRICE=$(bitmex ticker ETHUSD -o json 2>/dev/null | jq -r '.[].c[0] // "0"')
SOL_PRICE=$(bitmex ticker SOLUSD -o json 2>/dev/null | jq -r '.[].c[0] // "0"')
...
echo "  Buying BTC..."
bitmex paper buy BTCUSD "$BTC_VOL" -o json 2>/dev/null | jq -r '.order_id // .id // "done"'
echo "  Buying ETH..."
bitmex paper buy ETHUSD "$ETH_VOL" -o json 2>/dev/null | jq -r '.order_id // .id // "done"'
echo "  Buying SOL..."
bitmex paper buy SOLUSD "$SOL_VOL" -o json 2>/dev/null | jq -r '.order_id // .id // "done"'
echo ""

# Verify
echo "Step 5: Verify portfolio"
echo "Balances:"
bitmex paper balance -o json 2>/dev/null | jq '.'
echo ""
echo "Status:"
bitmex paper status -o json 2>/dev/null | jq '.'
echo ""
