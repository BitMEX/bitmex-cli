/// Built-in MCP (Model Context Protocol) server for bitmex-cli.
///
...
// WebSocket streaming is excluded from MCP tools (stateful, real-time only).
const STREAMING_GROUPS: &[&str] = &["ws"];
...
pub(crate) fn parse_services(input: &str) -> Result<Vec<String>> {
    ...
    if tokens.is_empty() {
        return Err(BitmexError::Validation {
            message: "At least one service must be specified".into(),
        });
    }
    ...
    for token in &tokens {
        if token == "utility" {
            return Err(BitmexError::Validation {
                message: "Service 'utility' is not available in MCP mode (interactive only)".into(),
            });
        }
...
mod tests {
    ...
    fn parse_explicit_list() {
        let result = parse_services("market,order").unwrap();
        assert_eq!(result, vec!["market", "order"]);
    }
    ...
    fn exclusions_remove_streaming_groups() {
        let input: Vec<String> = vec!["market".into(), "ws".into(), "order".into()];
