/// Derives JSON Schema from the catalog's `parameters` array.
///
/// This is the preferred path: the catalog expresses integer and boolean types
/// explicitly, giving richer schemas than clap introspection (which can only
/// produce `"string"` for numeric args). Falls back to [`clap_command_to_schema`]
/// when no catalog parameters are present.
pub(crate) fn catalog_parameters_to_schema(parameters: &[serde_json::Value]) -> serde_json::Value {
    ...
    for param in parameters {
        ...
        if super::schema::is_mcp_excluded_arg(&name.replace('-', "_")) {
            continue;
        }

        let mut prop = serde_json::Map::new();

        match param.get("type").and_then(|t| t.as_str()).unwrap_or("string") {
            "integer" | "number" => {
        ...
        if let Some(example) = param.get("example").and_then(|e| e.as_str()) {
            prop.insert("examples".into(), json!([example]));
        }

        if let Some(one_of) = param.get("oneOf").and_then(|o| o.as_array()) {
            prop.insert("oneOf".into(), serde_json::Value::Array(one_of.clone()));
        }
...
mod tests {
    ...
    fn excluded_commands() {
        ...
        assert!(!is_mcp_excluded_command("auth show"));
        assert!(!is_mcp_excluded_command("market orderbook"));
    }
